from typing import NamedTuple, Sequence, TypedDict, Union
...
def list_geom_ids_by_names(physics, geom_names, **render_kwargs):
    ...
    in_and_out = ([], [])
    for _id in geom_ids:
        in_and_out[not predicate(_id)].append(_id)
...
def camera_params_from_physics(physics, render_kwargs: CameraParams):
    width, height, camera_id = map(
        render_kwargs.get, ('width', 'height', 'camera_id'))
...
class PointCloudGenerator:
    ...
    def _mask(self, physics, point_cloud, render_kwargs):
        ...
        _, geom_ids = list_geom_ids_by_names(
            physics,
            self._bad_geom_names,
            **render_kwargs
        )
